...
def test_the_voice_is_given_the_plain_name_by_default():
    """No respelling unless one is asked for.

    "Gininda" was checked against a speech synthesiser and comes out right, so
    handing the voice anything else would be inventing a problem. The hook for
    a name that a voice does mangle stays; the default does not use it.
    """
    opening = greeting.opening()
    assert greeting.DEFAULT_OPERATOR in opening.spoken
    assert opening.hello in opening.spoken
