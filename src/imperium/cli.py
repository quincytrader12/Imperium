...
The Phase-1 deliverable is ``imperium balance``: read a key from the store, call
...
from imperium.logging_setup import mask
...
async def _client_for(cred, *, timeout: float = 15.0):
    ...
    from imperium.venues.registry import load_client_factory
...
def cmd_keys_list(args: argparse.Namespace) -> int:
    ...
    if not len(store):
        ...
        print("Add one with:  imperium keys add --name main --key <API-KEY> --secret <SECRET>")
        return 0
...
def cmd_keys_add(args: argparse.Namespace) -> int:
    ...
    print(f"    imperium keys enable {cred.name}")
    return 0
...
async def _balance(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        _print_err("")
        _print_err("  If you suspect the network rather than the key, run:  imperium diagnose")
        return 1
    finally:
        await client.aclose()
...
def cmd_calibrate(args: argparse.Namespace) -> int:
    from imperium.strategy.calibration import main as calibrate_main
