...
``godalgo/strategy/null_calibration.json``, which is *generated* by
``godalgo.strategy.calibration`` from simulated nulls. That indirection is the
...
def hurst_rs(returns: np.ndarray, min_chunk: int = 8) -> float:
    ...
    **This estimator is not centred on 0.5 in small samples.** On 250-bar random
    ...
    Returned as a raw number here; :mod:`godalgo.strategy.regime` converts it to
